...
sys_read(read_args_t *arg)
{
        ...
        read_args_t            kargs;
        int                     err;
        if ((err = copy_from_user(&kargs, arg, sizeof(read_args_t))) < 0) 
        {
                curthr->kt_errno = -err;
                return -1;
        }
        ...
        int retval;
        if( (retval = do_read(kargs.fd, temp_buffer, kargs.nbytes)) < 0 )
        {
                page_free(temp_buffer);
                curthr->kt_errno = -retval;
                return -1;
        }
        if ( (err = copy_to_user(arg->buf, temp_buffer, retval))< 0 ) 
        {
                page_free(temp_buffer);
                curthr->kt_errno = -err;
                return -1;
...
sys_write(write_args_t *arg)
{
        ...
        int cfu = copy_from_user(&write_arg, arg, sizeof(write_args_t));    
        /*if( cfu < 0) {
        ...
        size_t count = write_arg.nbytes;
        char *buff = (char*)(write_arg.buf);
        int total = 0;    
        while(count > 0) {
                size_t num = (count > PAGE_SIZE)?PAGE_SIZE:count;
                int cfu2 = copy_from_user(buf, buff, num);  
                /*if( cfu2 < 0) {
                        page_free(buf);
                        curthr->kt_errno = EFAULT;
                ...
                int write = do_write(write_arg.fd, buf, num);
                if(write < 0) {
                        page_free(buf);
...
{
        getdents_args_t dent_args;
        int errno = copy_from_user(&dent_args, arg, sizeof(getdents_args_t));
        /*if (errno < 0){
                curthr->kt_errno = -errno;
                return -1;
        }*/
        int i = 0, bytes_read = 0;
        dirent_t dir;
        int max_loop = dent_args.count / sizeof(dirent_t);
        
        while(i < max_loop){
            int bytes = do_getdent(dent_args.fd, &dir);
            if(bytes < 0){
            ...
            //char *addr = (char *)dent.dirp + i * sizeof(dirent_t);   
            int retVal = copy_to_user(dent_args.dirp + i, &dir, bytes);
            /*if(retVal < 0){
                curthr->kt_errno = -retVal;
                return -1;
            }*/
