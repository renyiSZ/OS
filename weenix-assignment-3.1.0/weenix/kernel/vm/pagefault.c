...
handle_pagefault(uintptr_t vaddr, uint32_t cause)
{
        ...
        pframe_t *page = NULL;
        uint32_t pnum = pagenum - vma->vma_start + vma->vma_off;
        if (pframe_lookup(vma->vma_obj, pnum, (cause & FAULT_WRITE)>>1, &page) < 0) {

            do_exit(EFAULT);
        }

        KASSERT(page);
        KASSERT(page->pf_addr);
        dbg(DBG_PRINT, "(GRADING3A 5.a)\n")

        if (cause & FAULT_WRITE) {
            pframe_pin(page);
            int dirty_res = pframe_dirty(page);
            pframe_unpin(page);
        ...
        pt_map(curproc->p_pagedir, (uintptr_t)PAGE_ALIGN_DOWN(vaddr), pt_virt_to_phys((uintptr_t)(page->pf_addr)), flag, flag);
