...
do_brk(void *addr, void **ret)
{
        ...
        void *brk = curproc->p_brk;
        void *brk_init = curproc->p_start_brk;
        ...
        // address should be larger than p_start_brk && less than USER_MEM_HIGH, otherwise should be error
        if (addr < brk_init || addr >= (void*)USER_MEM_HIGH) {
                return -ENOMEM;
        }

        uint32_t page_start = ADDR_TO_PN(PAGE_ALIGN_DOWN((uint32_t)brk_init));
        uint32_t page_end  = ADDR_TO_PN(PAGE_ALIGN_DOWN((uint32_t)addr-1))+1;


        vmarea_t *vma = vmmap_lookup(curproc->p_vmmap, page_start);

        if (vma == NULL) {
                return -ENOMEM;
        }

        /* addr should less than next occuring mapping. */
        vmarea_t *vma_next = list_item((vma->vma_plink.l_next), vmarea_t, vma_plink);


        if (page_end > vma_next->vma_start) {
                return -ENOMEM;
        }

        vma->vma_end = page_end;
        curproc->p_brk = addr;
        *ret = addr;
